...
const Page = () => {
  ...
  const removeKnownSkill = (skill: string) => {
    setValue(
      "knownSkills",
      knownSkills.filter((s) => s !== skill),
      { shouldValidate: true }
    );
  };

  const addWantToLearnSkill = (skill: string) => {
    setValue("skillsToLearn", [...skillsToLearn, skill], {
      shouldValidate: true,
    });
  };
      ...
      <div className="flex flex-col gap-4 items-center mb-[46px] w-full">
        ...
        <form className="w-full flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 z-10">
            ...
            <div className="flex gap-2 relative">
              <input
                ...
                className="w-full input"
                placeholder="e.g., Web Development, UI/UX Design, Data Analysis"
                id="password"
              />
              <button
                onClick={() => {
                ...
                type="button"
                className="button-violet text-sm! leading-5.5!"
              >
                Add skill
              </button>
              {inputKnown.length >= 1 && (
                  ...
                  <div className="mt-1 input p-2  min-w-[150px] max-w-[350px] flex gap-1 flex-wrap bg-white">
                    {!isPending ? (
                      availableSkills.length > 0 ? (
                        ...
                        <span className="text-sm leading-5 text-gray">
                          Empty
                        </span>
                      )
                    ) : (
            ...
            <div className="flex gap-1 flex-wrap">
              {knownSkills?.map((skill, idx) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    removeKnownSkill(skill);
                  }}
                  className="input cursor-pointer w-fit text-sm leading-5 py-1! transition-colors hover:bg-red-400 flex items-center gap-1"
                >
                  <span>{skill}</span>
                  <X size={14} />
                </button>
                        ...
                        <div className="flex flex-col gap-1">
                          {availableSkills.map((skill, idx) => (
                            <button
                              type="button"
                              onClick={() => {
                                addWantToLearnSkill(skill.title);
                                setInputToLearn("");
                              }}
                              key={idx}
                              className="input text-sm! cursor-pointer leading-5! font-medium transition-colors hover:bg-violet"
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2">
              <input
                {...register("checkBox")}
                className="checked:bg-violet"
                type="checkbox"
              />
              <label className="text-sm leading-5 font-medium">
                <span className="text-violet">Accept</span> Terms and Conditions
              </label>
            </div>
            {errors.checkBox && (
              <span data-testid="error" className="text-red-500 font-medium ">
        ...
        <div className="w-full">
          <button
            ...
            id="log-in-btn"
            className="button-violet mt-6 w-full"
          >
            Sign Up
          </button>
        </div>
        <div className="flex items-center gap-1 text-sm leading-5 ">
          ...
          <Link href={"/auth/login"} className="text-violet link">
            Log In
          </Link>
        </div>
      </div>
