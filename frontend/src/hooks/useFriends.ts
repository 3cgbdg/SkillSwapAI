...
export const useFriends = () => {
    ...
    // adding friend
    const { mutate: addFriend } = useMutation<{ id: string; message: string }, Error, { fromId: string; id: string }>({
        mutationFn: async ({ fromId, id }) => FriendsService.createFriend(fromId, id),
        onSuccess: (data) => {
    ...
    const { mutate: createFriendRequest } = useMutation<ApiResponse<null>, Error, { name?: string; id?: string }>({
