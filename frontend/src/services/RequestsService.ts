...
class RequestsService {
    ...
    async createFriendRequest({ name, id }: { name?: string, id?: string }): Promise<ApiResponse<null>> {
        const res = name ? await api.post("/requests", { name }) : await api.post("/requests", { id });
        return res.data;
    }
