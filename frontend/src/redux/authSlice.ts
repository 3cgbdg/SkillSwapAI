...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    addKnownSkill: (state, action: PayloadAction<string>) => {
      ...
      if (state.user?.knownSkills) state.user?.knownSkills.push(newItem);
    },
    addAiSuggestionSkills: (state, action: PayloadAction<string[]>) => {
      if (state.user) state.user.aiSuggestionSkills = action.payload;
    },
    ...
    deleteKnownSkill: (state, action: PayloadAction<string>) => {
      if (state.user?.knownSkills)
        state.user.knownSkills = state.user?.knownSkills?.filter(
          (skill) => skill.title !== action.payload
        );
    },
    deleteWantToLearnSkill: (state, action: PayloadAction<string>) => {
      if (state.user?.skillsToLearn)
        state.user.skillsToLearn = state.user?.skillsToLearn?.filter(
          (skill) => skill.title !== action.payload
        );
    },
    changeAvatar: (state, action: PayloadAction<string | undefined>) => {
      if (state.user) state.user.imageUrl = action.payload;
    },
    updateProfile: (
      state,
      action: PayloadAction<{ name?: string; bio?: string; email?: string }>
    ) => {
      if (state.user) {
