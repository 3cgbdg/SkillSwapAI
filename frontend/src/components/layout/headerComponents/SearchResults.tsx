...
const SearchResults = ({ foundSkills, foundUsers, onAddLearn, onCreateFriendRequest, onRemoveSkill }: SearchResultsProps) => {
    return (
                        ...
                        {foundSkills.map((skill, index) => {
                            return (
                                <div key={index} className="flex gap-2 items-center">
                                    ...
                                    <button onClick={() => { onAddLearn(skill.title, skill.id); onRemoveSkill(skill.id) }} className="btn  w-fit _border p-1 rounded-xl cursor-pointer transition-all hover:bg-green-400 outline-0" ><Plus size={20} /></button>
                                </div>
                            )
                        })}
                    </div>
                        ...
                        {foundUsers.map((user, index) => {
                            return (
                                <div key={index} className="flex gap-2 items-center">
                                    ...
                                    <button onClick={() => onCreateFriendRequest(user.id)} className="btn cursor-pointer  w-fit _border p-1 rounded-xl transition-all hover:bg-green-400 outline-0" ><Handshake size={20} /></button>
                                </div>
                            )
                        })}
                    </div>
