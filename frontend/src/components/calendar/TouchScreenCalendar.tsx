                    ...
                    <div className="flex flex-col gap-2">
                        {cell.sessions.length !== 0 ? cell.sessions.map(session => (
                            <div key={session.id} style={{ borderColor: session.color }} className="rounded-[10px] p-2 _border gap-1 flex flex-col ">
                                ...
                                <div className="">
                                    {session.description && <div className="border-y-1  border-gray-300 p-1 my-1">
                                        <div className="flex items-center justify-between">
                                            <h3 className="text-black font-medium">Description</h3>
                                        </div>
                                        <p className="text-gray text-xs leading-4">{session.description}</p>
                                    </div>}
                                    {session.meetingLink && <>
                                        <p className="text-gray text-xs leading-4">Meeting Link: <Link href={session.meetingLink} className="font-medium hover:underline text-black">{session.meetingLink}</Link> </p>
                                    </>}
                                </div>
                            </div>
                        )) :
