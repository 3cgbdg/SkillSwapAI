...
const DesktopGridCalendar = ({ tableCells }: { tableCells: TableCellType[] }) => {
    const [details, setDetails] = useState<null | { descr?: string, meetingLink: string | null }>(null)
        ...
        <div className="relative">
            {details !== null && (
                <div className="absolute -top-20 max-h-[250px] overflow-auto left-10 p-2 min-w-[150px] max-w-[300px]   _border rounded-md z-150 bg-white">
                    {details.descr && <div className="border-b-1 border-gray-300 pb-1 mb-1">
                        <div className="flex items-center justify-between">
                            <h3 className="text-black font-medium">Description</h3>
                            <button onClick={() => setDetails(null)} className="button-transparent"><X size={16} /></button>
                        </div>
                        <p className="text-gray text-xs leading-4">{details.descr}</p>
                    </div>}
                    {details.meetingLink && <>
                        <p className="text-gray text-xs leading-4">Meeting Link: <Link href={details.meetingLink} className="font-medium hover:underline text-black">{details.meetingLink}</Link> </p>
                    </>}
                </div>
            )}
                                    ...
                                    >
                                        <div className="flex items-center justify-between">
                                            ...
                                            {session.description != null &&
                                                <button onClick={() => setDetails({ descr: session.description, meetingLink: session.meetingLink })} className="hover:text-blue cursor-pointer underline text-lightBlue"><ReceiptText size={16} /></button>
                                            }
                                        </div>
