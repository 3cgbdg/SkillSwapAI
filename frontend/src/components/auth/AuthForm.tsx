...
const AuthForm: React.FC<AuthFormProps> = ({ mode }) => {
  ...
  return (
    <div
      className={`${isLogin ? "basis-[448px]" : "basis-[512px]"} w-full _border rounded-[32px] p-[40px] md:p-[57px] flex justify-center bg-white z-10 transition-all`}
    >
            ...
            <>
              ...
              <div className="flex flex-col gap-2 z-10">
                ...
                <div className="flex gap-2 relative">
                  ...
                  {inputKnown.length >= 2 &&
                    renderSkillSearch("known", inputKnown, setInputKnown)}
                </div>
                <div className="flex gap-2 flex-wrap">
                  {knownSkills.map((skill: string) => (
                    <span
                      key={skill}
                      className="bg-violet/10 text-violet px-3 py-1 rounded-full text-xs flex items-center gap-1 border border-violet/20"
                    >
                      {skill}
                      <X
                        size={12}
                        className="cursor-pointer hover:text-red-500"
                        onClick={() => removeSkill("known", skill)}
                      />
                    </span>
                  ))}
                </div>
                {errors.knownSkills && (
                  <span className="text-red-500 text-xs">
                    {errors.knownSkills.message as string}
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2 relative">
                  ...
                  {inputToLearn.length >= 2 &&
                    renderSkillSearch("learn", inputToLearn, setInputToLearn)}
                </div>
                <div className="flex gap-2 flex-wrap">
                  {skillsToLearn.map((skill: string) => (
                    <span
                      key={skill}
                      className="bg-cyan-500/10 text-cyan-600 px-3 py-1 rounded-full text-xs flex items-center gap-1 border border-cyan-500/20"
                    >
                      {skill}
                      <X
                        size={12}
              ...
              <div className="flex flex-col gap-2 mt-2">
                <div className="flex items-center gap-2">
                  ...
                  <label
                    htmlFor="terms"
                    className="text-sm text-gray font-medium cursor-pointer"
                  >
                    Accept{" "}
                    <span className="text-violet">Terms and Conditions</span>
                  </label>
                </div>
                {errors.checkBox && (
                  <span className="text-red-500 text-xs">
