...
const ComingSessionWarning = () => {
    ...
    useEffect(() => {
        ...
        const upcoming = sessions.find(item => {
            ...
            const sessionEnd = new Date(sessionStart.getTime() + 60 * (item.end - item.start) * 60 * 1000);

            return now >= sessionStartHourBefore && now <= sessionEnd || now >= sessionStart && now <= sessionEnd;
                    ...
                    <div className="flex flex-col ">
                        ...
                        )}
                        {
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <Link href={upComingSession.meetingLink} className="text-sm mt-1 hover:underline">{upComingSession.meetingLink}</Link>
                            </div>
                        }
                    </div>
                </div>
